...
class StaticShadowSnapshotSource:
    ...
    def next_snapshot(self) -> ShadowSnapshotInput | None:
        ...
        return ShadowSnapshotInput(
            ...
            snapshot_price=max(0.0, float(snapshot.snapshot_price)),
            execution_price_proxy=max(0.0, float(snapshot.execution_price_proxy)),
            requested_size=max(0.0, float(snapshot.requested_size)),
            snapshot_derived_liquidity=max(0.0, float(snapshot.snapshot_derived_liquidity)),
            observed_possible_fill=max(0.0, float(snapshot.observed_possible_fill)),
            path_complexity=max(0, int(snapshot.path_complexity)),
            route_instability=max(0.0, min(1.0, float(snapshot.route_instability))),
            competition_penalty=max(0.0, min(1.0, float(snapshot.competition_penalty))),
            slippage_estimate=max(0.0, float(snapshot.slippage_estimate)),
