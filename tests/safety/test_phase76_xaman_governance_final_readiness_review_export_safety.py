...
def test_phase76_module_has_no_network_wallet_archive_or_runtime_calls():
    combined = "\n".join(
        p.read_text(encoding="utf-8")
        for p in Path("src/sonic_xrpl/xaman_governance_final_readiness_review_export_spec").glob("*.py")
    )
    for token in [
        "requests.get(",
        "requests.post(",
...
def test_phase76_flags_block_export_runtime_payload_execution_wallet_mutation_and_bypass():
    f = _run("complete_spec_review_ready_export_package.json").spec.safety_flags
    assert f.spec_only is True
    assert f.review_export_spec_only is True
    assert f.runtime_export_service_allowed is False
