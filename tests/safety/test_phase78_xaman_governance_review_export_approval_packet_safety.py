...
def test_phase78_module_has_no_network_wallet_archive_or_runtime_calls():
    combined = "\n".join(
        p.read_text(encoding="utf-8")
        for p in Path("src/sonic_xrpl/xaman_governance_review_export_approval_packet_spec").glob("*.py")
    )
...
def test_phase78_flags_block_runtime_approval_payload_execution_wallet_mutation_and_bypass():
    f = _run("complete_spec_review_ready_approval_packet.json").spec.safety_flags
    assert f.spec_only is True
    assert f.approval_packet_spec_only is True
